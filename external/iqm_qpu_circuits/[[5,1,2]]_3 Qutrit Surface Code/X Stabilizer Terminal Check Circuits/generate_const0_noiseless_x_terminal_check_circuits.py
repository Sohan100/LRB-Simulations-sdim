...
def _find_repo_root(start: Path) -> Path:
    """Locate repository root by probing for current package layout."""
    for candidate in (start, *start.parents):
        if (candidate / "src" / "lrb" / "code_definitions.py").exists() and (
            candidate / "src" / "lrb" / "circuit_generator.py"
        ).exists():
            return candidate
    raise FileNotFoundError(
        "Could not locate repository root containing src/lrb package modules."
    )
...
from lrb import code_definitions as defns  # noqa: E402
from lrb.circuit_generator import (  # noqa: E402
    LRBCircuitGenerator,
    LRBCodeDefinition,
